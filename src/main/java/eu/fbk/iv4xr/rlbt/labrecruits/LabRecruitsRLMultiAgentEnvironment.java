...
//			this.entityList = new HashMap<String, Integer>();
	...
	public void startAgentEnvironment () throws InterruptedException {
		...
		//var dataCollectorpassive = new TestDataCollector();
		//testAgentPassive.setTestDataCollector(dataCollectorpassive);
		...
		//var dataCollectoractive = new TestDataCollector();
		//testAgentPassive.setTestDataCollector(dataCollectoractive);		

			
		// set the testing goal here  -- first activate the passive agent 
		GoalStructure goal =  explore(); //getActionGoal(goalEntity, goalEntityType);
		//DPrint.ul("Passive - Starting Simulation : \n  Goal : "+goalEntity +"    "+goal.toString() +"   Entity type : "+goalEntityType+"  status : "+goal.getStatus());
		...
		// create a test passive agent
		DPrint.ul ("agent goal status, passive = "+ testAgentPassive.success());		
		
		// set the testing goal here  -- first activate the passive agent
		//GoalStructure goalactive = getActionGoal("button1", LabEntity.SWITCH, testAgentActive);
		GoalStructure goalactive =  explore(); //getActionGoal(goalEntity, goalEntityType);
		//DPrint.ul("Active - Starting Simulation : \n  Goal : "+goalEntity +"    "+goalactive.toString() +"   Entity type : "+goalEntityType+"  status : "+goalactive.getStatus());
		doAction(goalactive, maxTicksPerAction, testAgentActive);		
		// run the agent until it solves its goal:
       // while (!(testAgentPassive.success() && testAgentActive.success())){
        
        //doAction(goal, maxTicksPerAction, testAgentPassive);
        	//doAction(goal, maxTicksPerAction, testAgentActive);
      //  }
		...
		DPrint.ul ("Initial State (passive Agent's view): "+ currentStatePassive.toString());
		DPrint.ul ("Initial State (active Agent's view): "+ currentStateActive.toString());
		
		DPrint.ul ("agent goal status, passive = "+ testAgentPassive.success()+"  active = "+testAgentActive.success());
	}
